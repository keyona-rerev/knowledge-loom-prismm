...
// Weekly newsletter health scan. pg_cron calls this once every 7 days with a
// shared secret (same pattern as fire-due-schedules). For every newsletter
// sender that's produced reference cards recently, this rolls up their
// scores into a recommendation: healthy, watch, or unsubscribe. Individual
// reference_cards are scored at ingestion time (see ingest-gmail-content +
// _shared/relevance-scorer.ts); this is the layer that turns "this one card
// was a 2" into "this sender has been a 2, 3, and 2 for the last three
// weeks running, consider dropping it."
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`✅ Newsletter health scan complete. ${rows.length} senders scanned, ${flagged} flagged.`);
